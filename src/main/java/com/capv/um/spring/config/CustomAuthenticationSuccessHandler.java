		...
		for (GrantedAuthority authorities : auths)
		{
		    if (authorities.getAuthority().equals("ROLE_ADMIN")){
		    	if(requestedUserRole != null && requestedUserRole.equals("admin"))
		    	{
		    		...
		    		response.sendRedirect(response.encodeURL("admin/adminhome"));
		    	}
		    	else
		    	...
		    	if(requestedUserRole != null && requestedUserRole.equals("manager"))
		    	{
		    		request.getSession(true).setAttribute("managername", authResult.getName());
		    		response.sendRedirect(response.encodeURL("manager/managerhome"));
		    	}
		    	else
		    		response.sendRedirect(response.encodeURL("login?error&role="+requestedUserRole));
		    } 
		    	...
		    	if(requestedUserRole != null && requestedUserRole.equals("user")) 
		    	{
		    		...
		    		response.sendRedirect(response.encodeURL("user/userhome"));
		    	}
		    	else
		    		response.sendRedirect(response.encodeURL("login?error&role="+requestedUserRole));
		    }
